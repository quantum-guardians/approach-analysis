...
def _run_raw_sa_trial(
    n: int,
    trial: int,
    seed: int | None,
) -> tuple[BasicSolverResult, TrialTimings]:
    graph, trial_seed, graph_time = _generate_trial_graph(n, trial, seed)
    timings = TrialTimings({"graph": graph_time})
...
def _run_global_trial(
    n: int,
    trial: int,
    seed: int | None,
) -> tuple[GlobalSolverResult, TrialTimings]:
    graph, _, graph_time = _generate_trial_graph(n, trial, seed)
    timings = TrialTimings({"graph": graph_time})
...
def _run_random_trial(
    ...
    start = time.monotonic()
    result = _calculate_random_baseline(graph, n, trial_seed)
